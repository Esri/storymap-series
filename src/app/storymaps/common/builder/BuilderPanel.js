		...
		return function BuilderPanel(container, builderSave, builderDirectCreationFirstSave, builderGalleryCreationFirstSave)
		{
			...
			function save()
			{
				...
				if ( container.find('.builder-save').hasClass('disabled') ) {
					if(app.appCfg.noAppTitleInitScreen && !app.data.getWebAppData().getTitle()){
			...
			this.incrementSaveCounter = function(/*nb*/)
			{
				...
				container.find(".builder-save").toggleClass("disabled",
					(app.isDirectCreationFirstSave && ! app.data.getStoryLength())
			...
			this.updateSharingStatus = function()
			{
				...
				if ( app.ui.mobileHeader )
					app.ui.mobileHeader.toggleSocialBtnAppSharing(disablePreview);
