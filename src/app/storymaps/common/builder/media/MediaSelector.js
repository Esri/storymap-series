	...
	function (
		...
		return function MediaSelector(container, mapCfg, smallSize, onDataChangeCallback, backButton)
		{
			...
			this.present = function(cfg)
			{
				...
				// Enable type selector if not only image
				container.find('.lblSelect, .media-types').toggle(!! (mapCfg || ! app.appCfg.mediaPickerDisableVideo || ! app.appCfg.mediaPickerDisableWebPage));

				if ( cfg.mode == "add" || cfg.mode == "import" )
					initUIAdd(cfg);
				else
					initUIEdit(cfg);
			};
			...
			this.getData = function()
			{
				...
				else if ( selectedMedia == 1 && _mode == "import")
					return _imageSelector.getData();
