define([
	...
	function(
		...
		return declare([],
		{
			...
			constructor: function(container, showView, noUI, hideResources) {
				this.showView = showView;
				noUI = noUI ? noUI : null;
				hideResources = hideResources ? hideResources : null;
				this.setupUI(container, noUI, hideResources);
			},
			...
			setupUI: function(container, noUI, hideResources) {
			...
			getTokenizedThumbUrl: function(resource) {
				...
				return CommonHelper.possiblyAddToken(resource.resourcesUrl + '/' + encodeURIComponent(fileName));
			...
			getEXIF: function(file, callback){
			...
			processFile: function(file) {
				...
				// this is currently only used in shortlist...
				if(app.appCfg.useImageEXIF && window.EXIF){
					this.getEXIF(file, function(data){
						var lat,
						lng,
						...
						if(exifData.GPSLatitude && exifData.GPSLatitude[0]){
							...
							// have to check Ref value and set to negative if South
							if(exifData.GPSLatitudeRef == "S")
								lat = -Math.abs(lat);
							file.lat = lat;
						}
						if(exifData.GPSLatitude && exifData.GPSLongitude[0]){
							lng = CommonHelper.convertToDegrees(exifData.GPSLongitude);
							// EXIF lat/lng values dont come in as true values
							// have to check Ref value and set to negative if West
				...
				fileUploadHelper.processFile(file, true).then(lang.hitch(this, function(result) {
					...
					if(app.appCfg.useImageEXIF){
						if(file.lat)
							_.assignIn(result, {lat: file.lat});
						if(file.lng)
							_.assignIn(result, {lng: file.lng});
					}
			...
			uploadFile: function(fileDetails) {
				fileUploadHelper.uploadSingleResource(fileDetails).then(lang.hitch(this, function(uploadResult) {
					...
					if(fileDetails.lat)
						uploadResult.lat = fileDetails.lat;
					if(fileDetails.lng)
						uploadResult.lng = fileDetails.lng;
					// try doing this to eliminate load time after configured...
					var img = new Image();
					img.src = CommonHelper.possiblyAddToken(uploadResult.picUrl || uploadResult.sizes[0].url);
					this.toggleLoadingIndicator(false);

					if(app.appCfg.mediaPickerSkipConfigure){
						topic.publish("IMAGEUPLOADED", uploadResult);
						return;
					}
