	...
	function (viewTpl, viewCss, topic) {
		return function ViewTheme()
		{
			...
			function renderThemesList(selectedThemeCfg)
			{
				...
				_themes = layoutProp[0].themes || [];

				if ( selectedTheme == -1 )
					selectedTheme = 0;

				// Switching layout and there is a matching theme
				if ( _selectedThemeGroup ) {
					$.each(_themes, function(index, theme){
						if ( _selectedThemeGroup == theme.group )
							selectedTheme = index;
					});
				}
				...
				if ( selectedThemeCfg ) {
					$.each(_themes, function(index, theme){
						if ( selectedThemeCfg.name == theme.name ) {
							selectedTheme = index;
							_selectedThemeGroup = theme.group;
						}
					});
				}

				$.each(_themes, function(i, theme){
					themesHTML += app.appCfg.getLayoutThumbnail({
						layout: _layout,
						theme: theme,
						options: _layoutCfg,
			...
			function onSelectTheme()
			{
				...
				_selectedThemeGroup = _themes[$(this).index()].group;
