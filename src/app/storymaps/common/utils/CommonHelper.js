		...
		return {
			...
			createGeocoder: function(p)
			{
				...
				if (allSources && allSources.length) {
					...
					searchOptions.placeholder = p.placeHolder || i18nMedia.commonMedia.editorActionGeocode.lblTitle;
				}
			...
			processGeocoderRequestReturn: function(responses, fallbackPlaceholder) {
				...
				$.each(responses, function(index, response){
					...
					var newSource = {
						...
						placeholder: targetGeocoder.placeholder || fallbackPlaceholder || i18nMedia.commonMedia.editorActionGeocode.lblTitle,
						locator: newLocator
					};
					newSource.name = newSource.name || self.findGeocoderName({url: newLocator.url});
					if (self.isEsriGeocoder({url: targetGeocoder.url})) {
						newSource.placeholder = i18nMedia.commonMedia.editorActionGeocode.lblTitle;
					}
				...
				$.each(appGeocoders, function(index, geocoder) {
					if (geocoder.url) {
						...
						if (self.isEsriGeocoder({url: optionsCopy.url})) {
							optionsCopy.placeholder = i18nMedia.commonMedia.editorActionGeocode.lblTitle;
			...
			setGeocodeSources: function(search, sources) {
				...
				search.startup();
				// set 'all' if there's no `appGeocoders` property
				if (!app.data.getWebAppData().getAppGeocoders) {
					return;
				}
				if (!app.data.getWebAppData().getAppGeocoders() && search._allIndex) {
					search.set('activeSourceIndex', search._allIndex);
				}
			},
			...
			convertToDegrees: function(value)
			{
				/*Helper function to convert the GPS coordinates stored in the EXIF to degress in float format*/
				var d0 = value[0].numerator;
				var d1 = value[0].denominator;
				var d = parseFloat(d0) / parseFloat(d1);

				var m0 = value[1].numerator;
				var m1 = value[1].denominator;
				var m = parseFloat(m0) / parseFloat(m1);

				var s0 = value[2].numerator;
				var s1 = value[2].denominator;
				var s = parseFloat(s0) / parseFloat(s1);
